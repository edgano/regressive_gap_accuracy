...
p<-ggplot(df, aes(AVGgapXseq, tc, shape=family, colour=family, fill=family)) +
  geom_smooth(method="lm") +
  geom_point(aes(colour = tree), size = 4)+
  theme_bw() + 
  xlab("% Gap") +
  ylab("TC score") +
...
df_select <- df %>% filter(family == "sdr")
p<-ggplot(df_select, aes(AVGgapXseq, tc, shape=aligner, colour=tree)) +
  geom_smooth(method="lm") +
  geom_point(size = 4)+
  theme_bw() + 
  xlab("% Gap") +
