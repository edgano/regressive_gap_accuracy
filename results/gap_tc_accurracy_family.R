...
province_list=list()
for (i in unique(df$family)) {
  
  province_number <- paste("Family",i)
  prov_filter <- filter(df, family == i)
  #print(prov_filter)
  
  plot <- ggplot(prov_filter, aes(AVGgapXseq, tc, shape=aligner, colour=tree)) + 
    geom_smooth(method="lm") +
    geom_point(size = 4)+
    theme_bw() + 
    #xlab("% Gap") +
...
##
# Plot non as fancy as possible
##
#install.packages("gridExtra")
#library(gridExtra)
#n <- length(province_list)
#nCol <- floor(sqrt(n))
#do.call("grid.arrange", c(province_list, ncol=3))
...
temp_legend <- ggplot(prov_filter, aes(AVGgapXseq, tc, shape=aligner, colour=tree)) + 
  geom_smooth(method="lm") +
  geom_point(size = 4)+
  theme_bw() + 
  #xlab("% Gap") +
...
main <- grid.arrange(grobs=province_list,ncol=5,bottom = legend)
main
