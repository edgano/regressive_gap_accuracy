...
province_list=list()
for (i in unique(df$aligner)) {
  
  province_number <- paste("aligner",i)
  prov_filter <- filter(df, aligner == i)
  #print(prov_filter)
  
  plot <- ggplot(prov_filter, aes(AVGgapXseq, tc, color=tree)) + 
    #geom_smooth(method="lm") +
    geom_point(size = 4)+
    theme_bw() + 
    #xlab("% Gap") +
...
temp_legend <- ggplot(prov_filter, aes(AVGgapXseq, tc, color=tree)) + 
  #geom_smooth(method="lm") +
  geom_point(size = 4)+
  theme_bw() + 
  #xlab("% Gap") +
